...
**Radek** (`r.brym@smartbox4you.com`) — autor a údržbář P2L Testeru, používá appku interně ve firmě Smart Product Solution / Smartbox4you. Pracuje na Windows 11 v Cursoru s rozšířením Codex (doma stroj uživatel `Brno`, v práci jiný — projekt cestuje přes git).
...
### Antivirus s HTTPS inspekcí rozbije `flutter build apk` (Gradle, PKIX)

**Symptom:** `flutter build windows` projde, ale `flutter build apk` spadne:
```
...
```
Flutter to shrne jako „Gradle threw an error while downloading artifacts from the network",
takže to vypadá na výpadek sítě — TCP na `dl.google.com:443` přitom projde normálně.

**Why:** Avast (a podobně ESET/Kaspersky) rozplétá HTTPS a podvrhuje vlastní certifikát:
```
dl.google.com  vydal: CN=Avast Web/Mail Shield Root, OU=generated by Avast Antivirus for SSL/TLS scanning
```
Windows aplikace to nepoznají — věří systémovému úložišti, kam si antivirus svůj kořen
nainstaloval. **Java má truststore vlastní** (`<JDK>\lib\security\cacerts`) a ten o něm neví,
takže padá jen Gradle. Odtud ta asymetrie: Windows build ano, APK ne.

**Fix** (bez admin práv a bez zásahu do repa — nastavení je uživatelské, druhý stroj se
nezmění): kopie `cacerts` do `%USERPROFILE%\.gradle\cacerts-avast` s doplněným kořenem
antiviru + `~/.gradle/gradle.properties` se `systemProp.javax.net.ssl.trustStore`.
Postup i podmínky jsou v komentáři toho `gradle.properties`. Alternativa na jedno použití:
vypnout v antiviru skenování HTTPS.

**Ověřit, kdo cert vydává** (než se začne hledat jinde):
```powershell
$t=New-Object Net.Sockets.TcpClient('dl.google.com',443)
...
### Cursor: `.cursorignore` rozbije Codex rozšíření

**Symptom:** Po restartu Cursoru se v projektu P2L-TESTER nenačte Codex rozšíření ani terminál.
...
- **`sqlite3` MUSÍ zůstat na 2.x.** Řada 3.x staví native lib přes Dart build hooks, které
  ...
  **Plán do budoucna (2026-08-07, zatím neprovedeno):** příčina není v balíčcích, ale
  v cestě k Flutter SDK — známý bug [flutter#173716](https://github.com/flutter/flutter/issues/173716).
  Řešení je přesunout SDK z `C:\Users\Radek Brym\flutter` do `C:\flutter` (a upravit PATH),
  načež jdou **oba** piny (`sqlite3` i `path_provider_foundation`) zrušit. Musí se to udělat
  na **obou** vývojových strojích — dokud tam mezera zůstane, piny musí zůstat, jinak build
  na druhém stroji po `git pull` selže. Na build hooks přechází víc balíčků, takže problém
  bude narůstat.
...
[lib/screens/changes_screen.dart](lib/screens/changes_screen.dart) nad `GET /api/units/history`
(+ `/history/filters` pro nabídky). Vstupy: ☰ na obrazovce Databáze → *Změny v databázi*, a ikona
`history` na kartě jednotky (s předvyplněným filtrem).

- **Endpointy musí být před `/:id`** — `/units/history` by jinak spadlo do detailu karty
  (`id='history'`). Stejná past jako u `/export`, `/changes`, `/sync`.
- **`hasMore` se pozná načtením o řádek víc** (`LIMIT n+1`), ne `COUNT` dotazem nad celou
  `unit_history` — ta poroste a počítat ji při každém scrollu je zbytečné.
- **Offline ukáže jen lokální změny** (`LocalUnitDb.recentHistory`) a napíše to — audit celé
  databáze je serverová veličina, lokální DB drží jen změny z tohohle zařízení.
- Filtry (`unitId`, `username` case-insensitive, `layer`, `origin`, časový rozsah) se skládají
  dynamicky do `WHERE`; `LIMIT`/`OFFSET` jdou do SQL jako konstanty (MariaDB je v prepared
  statementech přijímá nespolehlivě).

**Pozor na testy nad časem:** drift testy byly flaky, protože zapisovaly observed před desired a
spoléhaly na to, že se oba zápisy vejdou do stejné milisekundy (`computeDrift` má gate „viděno až
po změně evidence"). Časy v takových testech zadávat explicitně (helper `hourAgo()` v
`units.test.js`), ne je odvozovat od rychlosti stroje.

## Synchronizace lokální ↔ serverové DB (DB11)

[lib/services/sync_engine.dart](lib/services/sync_engine.dart) — `ChangeNotifier` singleton,
který **jen orchestruje**; práci dělá `UnitDbService` (`probeServer` / `pushOutbox` /
`pullFromServer`) nad `LocalUnitDb`. Web se nesynchronizuje (nemá lokální DB).

**Pořadí je push → pull**, ne naopak: server tak rozhoduje o konfliktu se znalostí obou verzí
a klient si hned stáhne výsledek včetně vlastní přehlasované karty.

**Triggery:** start appky (po `AuthSession.restore`), přihlášení za běhu (listener na
`AuthSession` — nativní login je opt-in), po lokálním zápisu (debounce 2 s, takže hromadná akce
nad 20 jednotkami = jeden sync), periodicky 10 min, po minutě při offline s neprázdnou frontou,
a ručně (ikona v AppBaru obrazovky Databáze).

**Pravidla, na která se naráží:**
- **Dostupnost serveru se pozná z OBSAHU odpovědi**, ne z HTTP statusu — captive portál
  zákazníkovy WiFi vrací `200 OK` s přihlašovací stránkou na cokoli. `probeServer` proto trvá
  na JSON s `ok: true` a `db` (timeout 3 s, ať UI nečeká).
- **Všechny čtyři výsledky pushe končí odebráním z fronty**: `applied`/`superseded` mlčky,
...
- **Konflikty se nemažou, jen `dismissed`** — dohledatelnost je smysl celé věci. UI (banner na
  kartě) nabízí „Poslat znovu" (nový zápis s aktuálním časem = legitimní výhra) a „Rozumím".
- **`source_device` bere `UnitDbService` z `LocalUnitDb.deviceLabel`** — hostname jde jen
  z `dart:io`, a service se kompiluje i pro web.
...
Current version: **2.89** (see `main.dart`)  
Recent themes:
- v2.89: **P2L LED dotažené podle reálné jednotky.** Navazuje na v2.88 — všechno níž vzniklo z ověřování na 1209 (FW 26071501NT). (1) **Barev je deset, ne šest** (`color_id` 0–9): `GET-CONFIG` vrací `color0`–`color9` i `color2_0`–`color2_9`, sloty 6–9 jsou z výroby černé. (2) **Tovární tabulka opravena podle firmwaru** — README-P2L.md u slotů 2–5 uvádí `ffff00`/`ff00ff`/`ffffff`, ale firmware má `aa5500`/`aa0055`/`555555`. Autoritativní je firmware, protože podle téhle tabulky se sloty vracejí na výchozí a zároveň slouží jako fallback, dokud jednotka neodpoví; hlídá to test. **Názvy slotů zůstaly z README** (`YELLOW` u hnědé, `WHITE` u šedé) — jsou to jména slotů v protokolu, ne popis barvy. (3) **Tlačítko „Obnovit výchozí"** v dialogu Barvy vrátí všech deset slotů na tovární hodnoty; jen předvyplní, odešle se až *Nastavit* (stejně jako „Obnovit" u DIST). Je na konci seznamu barev, ne v `actions` — `OverflowBar` tam tři tlačítka při úzkém dialogu zalomil na dva řádky. (4) **Víc rozsahů na jednom portu:** `SET-LEDS` je aditivní (předchozí úseky nezháší), takže zhasínání nemůže být „druhé klepnutí". Port je proto **vodorovně dělený vypínač** — nahoře číslo portu rozsvítí zadaný rozsah, spodní pruh zhasne celý port přes `CLR-STRIPS`. Barva horní části = poslední rozsvícení, drží se **per-port**, aby změna barvy nepřebarvila port svítící ještě tou původní. (5) **Rámeček tlačítka jde do `foregroundDecoration`** — v `decoration` ho barevné plochy překreslily a obsah vypadal, že leze přes okraj. (6) **Výchozí rozsah je `0`–`59`** místo `0`–`599`; povel na celý pásek zbytečně zatěžuje jednotku a na test stačí kus (kratší pásek si vezme svou délku). (7) **Jas z dialogu zapisuje `desired.brightness`** do evidence stejně jako hromadná akce — bez toho by karta hlásila falešný nesoulad. `SET-CONFIG` s jasem **ověřen naživo**: `{"brightness":100}` → `{"Code":0,"Message":"OK"}`. Počty LED a barvy se do evidence **záměrně nezapisují** (ladicí nastavení; důvod v docstringu obou metod). (8) **Zrušen přepínač „Vše / Zrušit"** nad porty — volal tutéž funkci jako primární tlačítko, takže byl druhým ovladačem téže akce s jiným popiskem (pozůstatek po přechodu na okamžité odesílání). (9) Paleta rychlé volby doplněna o odstíny z provozu. **Past, kterou odhalil test reálného payloadu:** klíč `color2` je barva **slotu 2**, ne „color2 slotu 0" — regexy parseru se nesmí přebít. Zůstává neověřené, jestli FW bere `SET-LEDS` jako **pole bloků** (na tom stojí rozsvícení víc portů jedním povelem) a co dělá `style_id 5`. Testy: 289 Flutter. Návod §5.
- v2.88: **P2L LED — ovládání LED pásků jednotky ze seznamu devices.** Nad kategoriemi devices přibyla sekce **P2L** s jediným chipem (číslo jednotky), který se **nepočítá mezi devices** — nejde o čip na RS485 sběrnici, ale o LED výstupy jednotky samotné (device kód `01`). Menu chipu: **Ovládání / Jas P2L LED / Počet P2L LED / Barvy P2L LED**, každá položka otevře modal ([p2l_led_dialogs.dart](lib/widgets/p2l_led_dialogs.dart)). Nepleť s LED kroužky na PUM-A/PUM-B — ty se dál ovládají z menu modulu. (1) **Dvojí protokol podle FW** ([command_service.dart](lib/services/command_service.dart)): FW ≥ `P2L_26071501NT` → povel v topicu `I/<unit>/P2L/01<addr>/<POVEL>` s plochým payloadem (`SET-LEDS`, `CLR-LEDS`, `CLR-STRIPS`, `SET-CONFIG`, `GET-CONFIG`), starší → tytéž hodnoty starým `CMD` formátem (`set_leds`/`clr_leds`/`clr_strips`/`set_brightness`/`set_led_count`/`set_color`). Payload obou variant staví jedna cesta, takže se nemůžou rozejít. (2) **Konfigurace se čte z jednotky** — `AppState.fetchP2lConfig` běží z `fetchDevices`, takže dialogy ukazují skutečný stav (jas, počty LED na portech, RGB barev). Model [p2l_led_config.dart](lib/models/p2l_led_config.dart) parsuje plochý payload (`brightness`, `leds port0`…, `color0`/`color2_0` jako hex); chybějící klíče doplní tovární hodnotou, aby neúplná odpověď nerozsypala slot na černou. **Past — kolize názvů povelu:** `GET-CONFIG` existuje pro UNIT (síťová konfigurace jednotky) i pro P2L (LED), takže routing v `_handleDeviceResponse` musí rozlišovat `parts[2]` — bez toho by LED odpověď spadla do UNIT handleru a přepsala observed vrstvu evidence. (3) **Ovládání:** LED od/do, barva, styl svícení a porty 0–7 ve dvou řadách. Barva je rozevírací seznam s barevným obdélníkem a číslem uvnitř, **RGB ze skutečné konfigurace**, ne z tabulky — přenastavení v *Barvy P2L LED* se propíše i sem. **Klepnutí na port povel rovnou odešle** (zhasnutý rozsvítí, svítící zhasne přes `CLR-STRIPS` na ten port), takže zvýrazněné tlačítko znamená „port svítí", ne „vybráno k odeslání". Barva se drží **per-port** (`Map<int,int>` port → `color_id`) — jinak by změna barvy v dialogu přebarvila i port, který fyzicky svítí ještě tou původní. (4) **Opraven latentní bug v `AppState.dispose()`** — rušil `MqttService` dřív, než se odhlásil z jeho streamů, takže poslední událost z `disconnect()` dorazila v mikroúloze do už zahozeného objektu (`notifyListeners()` po `dispose`). V provozu neškodil (AppState žije po celou dobu běhu appky), ale shazoval **každý** widget test, který AppState vytvoří. (5) **Dialogy jsou responzivní** — pevná šířka 380 px na úzkém okně přetékala (projevilo se zalomeným titulkem) a dropdowny si držely přirozenou šířku nejdelší položky; nově strop místo pevné šířky, `isExpanded` a zkracování textu. Pozn.: README-P2L.md má u `style_id` překlep — `3` chybí a `5` je uvedené dvakrát, do nabídky proto jdou `0,1,2,4,5,6,7,8`. **Částečně ověřeno na jednotce 1209** (FW 26071501NT): `SET-CONFIG` s `{"brightness":100}` vrátil `{"Code":0,"Message":"OK"}`. Neověřeno zůstává, jestli FW bere `SET-LEDS` jako **pole bloků** (na tom stojí rozsvícení víc portů i víc rozsahů najednou) a co dělá `style_id 5`. Jas z dialogu zapisuje `desired.brightness` do evidence stejně jako hromadná akce — bez toho by karta hlásila falešný nesoulad. Testy: 286 Flutter, z toho nové [p2l_led_test.dart](test/p2l_led_test.dart) (parser + oba protokoly) a [p2l_led_dialogs_test.dart](test/p2l_led_dialogs_test.dart). Návod §5.
